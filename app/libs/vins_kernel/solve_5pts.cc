...
// 通过对极约束计算本质矩阵 E
// 并从 E 中恢复出当前帧与第l帧之间的相对变换R, t(以第一帧图像相机坐标系为参考坐标系), 并判断内点数目是否足够
bool MotionEstimator::solveRelativeRT(const vector<pair<Vector3d, Vector3d>> &corres, Matrix3d &Rotation, Vector3d &Translation)
{
    if (corres.size() >= 15)
    {
        ...
        cv::Mat E = cv::findFundamentalMat(ll, rr, cv::FM_RANSAC, 0.3 / 460, 0.99, mask); // 用于求解基础矩阵, 当然也可用于求解基础矩阵
        cv::Mat cameraMatrix = (cv::Mat_<double>(3, 3) << 1, 0, 0, 0, 1, 0, 0, 0, 1);
        cv::Mat rot, trans;
        // 从 E 中恢复出当前帧与第l帧之间的相对变换R, t(以第一帧图像相机坐标系为参考坐标系)
